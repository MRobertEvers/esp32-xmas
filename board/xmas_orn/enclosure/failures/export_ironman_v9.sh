...
mkdir -p stl/ironman_v9 docs
for colour in body gold black white; do
    target="stl/ironman_v9/ironman_${colour}.stl"
    "$OPENSCAD" --hardwarnings -q --export-format binstl \
        -D "part=\"${colour}_print\"" -o "$target" ironman_v9.scad
    echo "$target"
done
"$OPENSCAD" --hardwarnings -q --projection=o --colorscheme=Tomorrow \
    --imgsize=1100,1100 --camera=13.75,27,8.8,0,0,0,330 \
    -o docs/ironman_v9_front.png ironman_v9.scad
"$OPENSCAD" --hardwarnings -q --projection=o --colorscheme=Tomorrow \
    --imgsize=1100,1100 --camera=13.75,27,8.8,23,16,0,340 \
    -o docs/ironman_v9_iso.png ironman_v9.scad
echo "docs/ironman_v9_front.png"
echo "docs/ironman_v9_iso.png"
