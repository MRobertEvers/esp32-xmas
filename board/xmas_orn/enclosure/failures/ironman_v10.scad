// Iron Man v10: return to the compact V5 front, with a plain lower chest.
// No raised neck. Side accents suggest upper arms and elbow joints within the V5 outline.
// Previous versions and the common core interface are preserved.
// part: ironman_v10 | body/gold/black/white[_print] | check_core/colours/backing
include <xmas_orn_case.scad>
part = "ironman_v10";
preview_display = true;
reactor_shape = "circle"; // circle (V5) or trapezoid
assert(reactor_shape == "circle" || reactor_shape == "trapezoid");
...
// Original V5 chest outline: sloped shoulder plates, recessed collar and tapered waist.
module im_outline() polygon([
    [-9,31],[9,31],[14,38],[29,40],[41,29],[39,13],[34,5],
    [32,-15],[24,-35],[-24,-35],[-32,-15],[-34,5],[-39,13],
    [-41,29],[-29,40],[-14,38]]);
module im_reactor() {
    if (reactor_shape == "circle") translate([0,4]) circle(r=24.5,$fn=160);
    else polygon([[-28,25],[28,25],[18,-19],[-18,-19]]);
}
module im_gold() im_pair() {
    // Compact gold upper-arm plates, following the existing sides of the chest.
    polygon([[31,12],[34,16],[33,1],[30,-8],[27,-10],[28,-3]]);
}
module im_black() {
    offset(delta=2) im_reactor();
    im_pair() {
        im_line([[12,29],[19,31],[29,33],[35,27]],1.1);
        im_line([[36,22],[35,16]],1.1);
        // A small elbow joint distinguishes the red lower arm from the gold upper arm.
        polygon([[27,-12],[31,-10],[30,-13],[26,-15]]);
    }
    // Intentionally no neckline insert or lines across the lower central panel.
}
