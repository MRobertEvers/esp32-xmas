// V2c: V2b plus ice-blue eye lenses and a matching reactor frame.
// Retains V2's complete Iron Man sleeve geometry; replaces only the front colour layout.
// part: ironman_v2c | body/gold/black/cyan[_print] | check_core/colours/backing/baseline
include <xmas_orn_outers_v2.scad>
part = "ironman_v2c";
preview_display = true;
fresh_names = ["body", "gold", "black", "cyan"];
fresh_colours = ["#a91f2c", "#e4b44f", "#24232b", "#70e2ff"];
...
module fresh_eyes() v2_bilateral() polygon([
    [-19,33],[-5.5,30.5],[-7,28],[-18,29]]);
module fresh_black() {
    offset(delta=1.2) fresh_reactor();
    offset(delta=0.65) fresh_eyes();
}
// The reactor and both eye lenses use a single ice-blue filament (four colours total).
module fresh_cyan() {
    difference() { fresh_reactor(); offset(delta=-1.5) fresh_reactor(); }
    fresh_eyes();
}
module fresh_raw(c) {
    if (c == 1) fresh_gold();
